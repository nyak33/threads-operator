"""Command-line entrypoints for the threads-operator toolkit.

All commands are idempotent and never mutate browser state beyond a single
clean page load for Activity collection.
"""
...
def _setup_logging(level: str = "WARNING") -> None:
    ...
    logging.basicConfig(level=getattr(logging, level.upper(), logging.WARNING), format=fmt)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="threads-operator", description="Threads Insights & Activity Follow collector")
    ...
    ap = sub.add_parser("activity-follow", help="Read /activity/follows and store results")
...
def cmd_insights(env: dict[str, str], args: argparse.Namespace) -> int:
    ...
    store = SupabaseStore(base_url=env["SUPABASE_URL"], service_role_key=env["SUPABASE_SERVICE_ROLE_KEY"])
    try:
        summary = collect_once(api, store, account_sample_minutes=int(env.get("THREADS_ACCOUNT_SAMPLE_MINUTES", "15")))
...
def cmd_activity_follow(env: dict[str, str], args: argparse.Namespace, profile_dir: pathlib.Path) -> int:
    store = SupabaseStore(base_url=env["SUPABASE_URL"], service_role_key=env["SUPABASE_SERVICE_ROLE_KEY"])
    print(f"[activity-follow] reading {ACTIVITY_URL} …")
    result = collect_activity_follows(store=store, profile_dir=profile_dir, own_posts=None,
                                      settle_seconds=args.settle, persist=not args.dry_run)
    ...
    if args.dry_run and ok:
        ...
        return 0
    if ok:
        print(f"[activity-follow] done — {total} events, {new_events} new, {high}H/{medium}M/{low}L/{unknown}U")
        return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    profile_dir = pathlib.Path(env.get("THREADS_BROWSER_PROFILE", "~/.hermes/browser-profiles/threads")).expanduser()
    try:
        if args.command == "insights":
            return cmd_insights(env, args)
        if args.command == "activity-follow":
            enabled = env.get("ACTIVITY_FOLLOW_COLLECTOR_ENABLED", "false").lower() in {"1", "true", "yes", "on"}
            if not args.dry_run and not enabled:
                print("Activity Follow Collector is disabled; set ACTIVITY_FOLLOW_COLLECTOR_ENABLED=true to persist.", file=sys.stderr)
                return 3
