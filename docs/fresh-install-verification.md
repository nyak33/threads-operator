...
| Supabase migrations on a fresh project | EXTERNALLY VERIFIED (2026-09-23) | User replayed the full chain (`013,001,001b,002,003,004,005,006,007,008,009,010,011,012`) against a throwaway Supabase project: all 14 applied cleanly, idempotency re-run passed with zero errors, view/4 FKs/`backlog`+`discarded` CHECK verified. **But it surfaced a fresh-install security gap** — see defect #3 below. `002_post_daily_rollups.sql` also confirmed applied to production (the `rebuild_rollups.py` blocker cleared). |
