...
| 2026-09-23 | Insights wrapper keeps the runtime key-injection behavior but via bash nameref (no literal `SUPABASE_SERVICE_ROLE_KEY=<value>` text) | Preserves production behavior AND keeps `tests/test_no_secrets.py` strict — test NOT weakened; baseline restored to 542 passed / 1 skipped. |
